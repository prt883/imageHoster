...
public class ImageController {
    ...
    public String showImage(@PathVariable("id") Integer id,@PathVariable("title") String title, Model model) {
        ...
        model.addAttribute("comments",image.getComments());// To display comments
    ...
    public String editImage(@RequestParam("imageId") Integer imageId, Model model, RedirectAttributes redir, HttpSession session) {  //added session and redirectattribute
        Image image = imageService.getImage(imageId);
        User existingUser=(User) session.getAttribute("loggeduser");
        if(existingUser.getId()!=image.getUser().getId()){                      //checking if the user is the owner of the image or not
            String error="Only the owner of the image can edit the image";
            redir.addFlashAttribute("editError", error);
    ...
    public String deleteImageSubmit(@RequestParam(name = "imageId") Integer imageId, RedirectAttributes redir, HttpSession session) {  //added session and redirectattribute

        Image image = imageService.getImage(imageId);
        User existingUser=(User) session.getAttribute("loggeduser");
        if(existingUser.getId()!=image.getUser().getId()){                          //checking if the user is owner of the image or not
            String error="Only the owner of the image can delete the image";
            redir.addFlashAttribute("deleteError", error);
            return "redirect:/images/"+imageId+"/"+image.getTitle();
        }
